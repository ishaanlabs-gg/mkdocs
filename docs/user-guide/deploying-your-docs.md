...
You can also publish Project Pages automatically with [GitHub Actions]. For
example, add the following workflow to `.github/workflows/docs.yml` to deploy
...
Adjust the branch name if your default branch is not `master`. If your project
uses third-party themes or plugins, install them in the workflow before running
`mkdocs gh-deploy`.
...
[GitLab Pages] can host the static files generated by MkDocs. Add a
`.gitlab-ci.yml` file similar to the following:
...
GitLab publishes the contents of the `public` directory from the `pages` job.
If your project uses third-party themes or plugins, install them before running
`mkdocs build`.
